...
#SBATCH --job-name=token_pos_trad
#SBATCH --output=logs/token_pos_trad_%A_%a.out
#SBATCH --error=logs/token_pos_trad_%A_%a.out
#SBATCH --array=0-7
...
apptainer exec --nv -B $XDG_CACHE_HOME mitra.sif python3.12 src/token_position/translation.py $1 $source $target $index
