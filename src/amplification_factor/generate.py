...
def main(
    ...
    if Path(
        f"results/amplification_factor/{model_name.split('/')[-1]}:{trad_source}:{trad_target}:{lang_source}:{lang_target}:{num_trad_heads}:{num_lang_heads}:{str(amplification_factor).replace('.', '_')}.csv"
    ).exists():
    ...
    h = add_vectors(h_lang, h_trad, factor_v1=amplification_factor, factor_v2=amplification_factor)
    ...
    Path("results/amplification_factor").mkdir(parents=True, exist_ok=True)
    results_df.to_csv(
        f"results/amplification_factor/{model_name.split('/')[-1]}:{trad_source}:{trad_target}:{lang_source}:{lang_target}:{num_trad_heads}:{num_lang_heads}:{str(amplification_factor).replace('.', '_')}.csv",
        index=False,
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 9:
        print(
            "Usage: python generate.py <model_name> <trad_source> <trad_target> <lang_source> <lang_target> <% of trad_heads> <% of lang_heads> <amplification_factor>"
        )
        sys.exit(1)
